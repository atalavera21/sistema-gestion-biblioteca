# Arquitectura y Decisiones Tecnicas — Sistema de Gestion de Biblioteca Universitaria
...
### ADR-011: Apache PDFBox como Motor de Reportes PDF

**Estado:** Aceptado (2026-05-02)

**Contexto:**
JasperReports 6.20.x, originalmente planificado, depende transitivamente de artefactos JAXB legacy (`javax.xml.bind:jaxb-api`, `com.sun.xml.fastinfoset:FastInfoset`) alojados en repositorios `http://maven.java.net` bloqueados por Maven 3.9+. Forzar versiones Jakarta y exclusiones resultó en una cadena infinita de dependencias rotas.

**Decision:**
Reemplazar JasperReports por Apache PDFBox 3.0.4 para generación de PDFs.

**Alternativas:**
| Alternativa | Pros | Contras |
|---|---|---|
| JasperReports 7.x | Migración mínima | Limitado en Maven Central, no probado |
| Apache FOP | Estándar W3C XSL-FO | Requiere plantillas XSL |
| iText 7 | Muy potente | Licencia AGPL |

**Consecuencias:**
- ✅ Cero dependencias externas. Sin conflictos JAXB, sin repositorios bloqueados.
- ✅ API programática simple. Se generan PDFs con tablas, títulos y paginación automática directamente desde consultas JPQL.
- ❌ No usa plantillas JRXML (pero para el alcance actual no era necesario).
- ❌ Cambio de API — `ReporteService` escrito desde cero con PDFBox.

---

### PENDIENTE: ADR-012 — Refactorizar Categoria como Entidad

**Estado:** Propuesto (planificado para sesion 3)

**Contexto:**
Actualmente `Libro.categoria` es un `String`. Esto impide:
- CRUD de categorías desde el panel admin
- Validación de categorías existentes
- Reportes por categoría con datos normalizados
- Relaciones: `Categoria 1→* Libro`

**Decision propuesta:** Crear entidad `Categoria` (id, nombre) con relación `@ManyToOne` desde `Libro`.
...
| Reportes mas complejos | Los mismos DAOs alimentan tanto PDF (PDFBox) como Excel (POI). JPQL escala a consultas avanzadas. |
